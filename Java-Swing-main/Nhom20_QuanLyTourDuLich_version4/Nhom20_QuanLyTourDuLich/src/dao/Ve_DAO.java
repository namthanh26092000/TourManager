	...
	public ArrayList<Ve> getVeTheoMaTour(String maTour){
		...
		try {
			...
			while (rs.next()) {
				...
				Date ngayDatVe = rs.getDate(2);
				//Tour  tour = new Tour(rs.getString(5));
				Ve ve = new Ve(maVe,ngayDatVe,new Tour(rs.getString(7)),new KhachHang(rs.getString(19)),new NhanVien(rs.getString(26)));
				dsVe.add(ve);
	...
	public ArrayList<Ve> DanhSachVeTheoMaTour(String maTour){
		...
		try {
			...
			while (rs.next()) {
				...
				Ve ve = new Ve(maVe,ngayDatVe,khachHang, nhanVien,tour);
				dsVe.add(ve);
